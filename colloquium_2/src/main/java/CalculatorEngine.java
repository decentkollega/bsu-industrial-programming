public class CalculatorEngine {
    ...
    private void convertToDouble(String number, Operator operator) {
        ...
        switch (operator) {
            case ADD -> add(dblNumber);
            case SUBTRACT -> subtract(dblNumber);
            case MULTIPLY -> multiply(dblNumber);
            case DIVIDE -> divide(dblNumber);
            default -> throw new AssertionError(operator.name());
        }
    }
